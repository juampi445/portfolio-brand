...
// Effect inspired by Paper's Liquid Metal effect (React Bits, MetallicPaint).
// Kept faithful to the upstream shader; the React shell adds two things this
// project needs because it mounts THREE of these at once (one per service card):
//   1. a WEBGL_lose_context release on teardown — the browser caps live WebGL
//      contexts (~16 across all tabs) and Fast Refresh would otherwise orphan
//      one on every reload until the cap is hit (see SideRays for the same fix);
//   2. an IntersectionObserver gate so off-screen canvases don't burn a rAF
//      loop + full-frame redraw while the Services section is out of view.
...
interface MetallicPaintProps {
  ...
  className?: string;
  /**
   * Internal square render size in CSS px before DPR. Kept small here because
   * the icons paint at ~5rem — a 1000px buffer per card (three of them) is pure
   * GPU waste at that scale. The shader works in normalized coords, so this only
   * trades crispness, not the look.
   */
  resolution?: number;
}

// The depth field processImage() derives is a pure function of the source image
// — it never changes with locale, theme or anything else. Yet a language switch
// remounts the whole section and would recompute all three from scratch (a
// 200-iteration relaxation each, synchronous on the main thread — the visible
// freeze). Cache the result by imageSrc at module scope so it survives remounts:
// computed once per session, reused instantly on every switch after. Keyed on
// the data URI, which is identical across locales, so all three keys are stable.
...
export default function MetallicPaint({
  ...
  // Bumped when the browser restores a reclaimed context, to force a clean
  // re-init below (a language switch remounts all three at once — see effect).
  ...
  const initGL = useCallback(() => {
    ...
    const compile = (src: string, type: number): WebGLShader | null => {
      ...
      if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        // A context lost mid-init — routine during Fast Refresh / remount churn
        // when the browser reclaims a WebGL context — fails EVERY compile with
        // an empty info log. That's transient noise, not a shader bug (the
        // retry rebuilds once a context is free), so stay silent when the
        // context is gone; only surface a genuine error on a live context.
        if (!gl.isContextLost()) console.error(gl.getShaderInfoLog(s));
        return null;
      }
      return s;
    };
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      if (!gl.isContextLost()) console.error(gl.getProgramInfoLog(prog));
      return false;
    }
  ...
  useEffect(() => {
    ...
    // The real "icons break on switch" cause: a language switch remounts this
    // whole section, and the App Router commits the NEW tree before unmounting
    // the OLD one — so for a moment SIX MetallicPaint contexts (three old +
    // three new) are live at once, on top of SideRays and RocketModel. That
    // blows the browser's ~16-context cap, getContext returns null, and the
    // icon stays blank with no context to even fire lost/restored on.
    //
    // Fix, mirroring SideRays: DON'T grab the context synchronously in this
    // commit. Defer a tick so the old tree unmounts and frees its three first,
    // and if the cap is still hit, back off and retry — the pressure is
    // transient, so a couple of frames later getContext succeeds.
    let cancelled = false;
    ...
    const tryInit = () => {
      ...
      if (!initGL()) {
        // Almost always the transient over-cap during a remount; retry a few
        // times before giving up (the mark is decorative, so blank-forever is
        // the only outcome we must avoid, and this prevents it).
        if (attempts++ < 12) timer = setTimeout(tryInit, 150);
        return;
      }
      ...
      const side = Math.round(resolution * Math.min(window.devicePixelRatio || 1, 2));
      canvas.width = side;
      canvas.height = side;
      gl.viewport(0, 0, side, side);

      // If the browser still reclaims this context later (a new tab needs one),
      // recover: preventDefault marks intent to restore, and the restore bumps
      // the epoch to rebuild context + texture from scratch.
      lostHandler = (e: Event) => {
        e.preventDefault();
      ...
      restoredHandler = () => setContextEpoch((n) => n + 1);
    ...
    // One tick past this commit, so a remount's old contexts release first.
    timer = setTimeout(tryInit, 10);

    return () => {
      cancelled = true;
      if (timer) clearTimeout(timer);
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
      // Drop the listeners before releasing, so the synthetic "lost" event that
      // loseContext() fires can't re-enter the handler above (SideRays does the
      // same).
      if (lostHandler) canvas.removeEventListener('webglcontextlost', lostHandler);
      if (restoredHandler) canvas.removeEventListener('webglcontextrestored', restoredHandler);
      const ctx = glRef.current;
      if (ctx) {
        if (textureRef.current) ctx.deleteTexture(textureRef.current);
        // Free the GPU context now instead of waiting for GC, so the next mount
        // isn't the one that tips over the cap.
        ctx.getExtension('WEBGL_lose_context')?.loseContext();
      }
      glRef.current = null;
    };
  }, [initGL, resolution, contextEpoch]);
  ...
  useEffect(() => {
    ...
    // Cache hit (any switch after the first): skip the image decode AND the
    // relaxation entirely, just re-upload the stored field to the new context.
    // This is what keeps a language switch from re-freezing the main thread.
    const cached = imageDataCache.get(imageSrc);
    if (cached) {
    ...
    img.onload = () => {
      // The load is async; the component may have been torn down (remount on a
      // language switch) between here and now. Uploading then would push a
      // texture into a context that no longer exists.
      if (cancelled) return;
      const imgData = processImage(img);
      imageDataCache.set(imageSrc, imgData);
  ...
  // Skip the GPU redraw while the card is scrolled out of view — three always-on
  // rAF loops redrawing a full frame each is wasted work off-screen. This is a
  // ref, not state, on purpose: it gates the draw INSIDE the loop, so the loop
  // still starts and paints at least one frame regardless of visibility. Gating
  // the whole effect on a `visible` state risked it being false at the instant
  // the texture became ready, leaving the canvas blank forever.
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
  ...
  useEffect(() => {
    ...
    // Draw at least the first frame no matter what; after that, skip the GPU
    // redraw while off-screen (visibleRef) but keep the loop alive so it resumes
    // instantly when the card scrolls back in.
    let drawnOnce = false;
    const render = (time: number) => {
      const delta = time - lastTimeRef.current;
      lastTimeRef.current = time;

      if (visibleRef.current || !drawnOnce) {
